...
public class Index {
    ...
    public static String oldFileName = "C:/Sarnia.xls";
...
//
        for (int i = 0; i < tracts.size(); i++) {
            ...
            if (tracts.get(i).getUsed() == false){
                System.out.println(tracts.get(i).getName());
                myMethod(i);
            }
        }
//
...
//        //if (((Tract)tract.Getelementbypos(tractNo)).getOwnerStatus().contains("OWNER")){
        newSheet.getRow(row).getCell(0).setCellValue(findMultiTracts(index));
//        //}
//        try{
//            newSheet.getRow(row).getCell(15).setCellValue(relativeNotes(tractNo));
//        }
